...
docker buildx build --load --platform linux/amd64 -t "test:latest" .
docker build --platform linux/amd64 -t "${OCR_PADDLE_IMAGE_NAME}:latest" -f src/AiPipeline.TireOcr/AiPipeline.TireOcr.PreprocessingPaddle/Dockerfile .
