...
docker build --platform linux/amd64 -t "${OCR_IMAGE_NAME}:latest" -f src/AiPipeline.TireOcr.Ocr/AiPipeline.TireOcr.Ocr.WebApi/Dockerfile .
...
#docker buildx create --use --name mybuilder
docker buildx inspect --bootstrap
docker build --platform linux/amd64 -t "${OCR_PYTHON_IMAGE_NAME}:latest" -f src/AiPipeline.TireOcr.PythonOcr/Dockerfile .
...
docker build --platform linux/amd64 -t "${EVALUATION_TOOL_IMAGE_NAME}:latest" -f src/AiPipeline.TireOcr.EvaluationTool/AiPipeline.TireOcr.EvaluationTool.WebApi/Dockerfile .
echo "Successfully built ${EVALUATION_TOOL_IMAGE_NAME}"
echo ""

# EVALUATION_TOOL_FE
echo "Building image for evaluation tool frontend"
docker build --platform linux/amd64 -t "${EVALUATION_TOOL_FE_IMAGE_NAME}:latest" -f src/AiPipeline.TireOcr.EvaluationTool.Ui/Dockerfile ./src/AiPipeline.TireOcr.EvaluationTool.Ui
echo "Successfully built ${EVALUATION_TOOL_FE_IMAGE_NAME}"
echo ""

# TIRE_DB_MATCHER
echo "Building image for tire db matcher service"
docker build --platform linux/amd64 -t "${TIRE_DB_MATCHER_IMAGE_NAME}:latest" -f src/AiPipeline.TireOcr.TasyDbMatcher/AiPipeline.TireOcr.TasyDbMatcher.WebApi/Dockerfile .
