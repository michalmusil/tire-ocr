...
from src.dependencies import (
    get_perform_preprocessing_3_command_handler,
    get_perform_preprocessing_4_command_handler,
)
...
@router.post("/v4/file")
# add annotation for UploadFile
async def preprocessing_v4_file(
    image: Annotated[
        UploadFile, File(description="Image file to perform the preprocessing on.")
    ],
    handler: Annotated[
        PerformPreprocessing4CommandHandler,
        Depends(get_perform_preprocessing_4_command_handler),
    ],
):
    try:
        image_bytes = await image.read()
        command = PerformPreprocessing4Command(image=image_bytes)
        result = await handler.handle(command)
...
@router.post("/v4")
async def preprocessing_v4_file_json(
    image: Annotated[
        UploadFile, File(description="Image file to perform the preprocessing on.")
    ],
    handler: Annotated[
