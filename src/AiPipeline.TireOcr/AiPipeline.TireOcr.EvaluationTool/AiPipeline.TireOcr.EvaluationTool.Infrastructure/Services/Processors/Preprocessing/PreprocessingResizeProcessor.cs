...
public class PreprocessingResizeProcessor : IPreprocessingProcessor
{
    private readonly HttpClient _remoteProcessorClient;
    private readonly ILogger<PreprocessingResizeProcessor> _logger;

    public PreprocessingResizeProcessor(HttpClient remoteProcessorClient,
        ...
        PreprocessingType preprocessingType)
    {
        try
        {
            ...
            content.Add(new StringContent("2048"), "MaxSidePixels");

            var res = await _remoteProcessorClient.PostAsync("/api/v1/Preprocess/ResizeToMaxSide", content);
