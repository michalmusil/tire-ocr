...
namespace AiPipeline.Orchestration.Runner.Infrastructure.Services;
...
public class PipelineBuilderService : IPipelineBuilderService
{
    ...
    public async Task<DataResult<Pipeline>> BuildAsync()
    {
        if (_pipelineInput is null)
            return DataResult<Pipeline>.Failure(NoInputProvidedFailure);
        ...
        for (var i = 0; i < _steps.Count; i++)
        {
            ...
            if (node is null)
                return DataResult<Pipeline>.NotFound($"Node type {requestedStep.NodeId} not found");

            var procedure = node.AvailableProcedures.FirstOrDefault(p => p.Id == requestedStep.ProcedureId);
            if (procedure is null)
                return DataResult<Pipeline>.NotFound(
            ...
            if (isFirstStep)
            {
                ...
                if (inputValidationResult.IsFailure)
                    return DataResult<Pipeline>.Failure(inputValidationResult.Failures);
            }
        ...
        var pipeline = new Pipeline(steps: pipelineSteps);
        var validationResult = pipeline.Validate();
        return validationResult.Map(
            onSuccess: () => DataResult<Pipeline>.Success(pipeline),
            onFailure: DataResult<Pipeline>.Failure
        );
    }

    private async Task<List<NodeType>> GetNodeTypesForAllStepsAsync()
    {
